...
void  W25Q64_CS_GPIO_Init(void)
{
    ...
    W25Q64_CS_GPIO_CLK_EN();
    GPIO_InitStruct.Pin   = W25Q64_CS_PIN;
    GPIO_InitStruct.Mode  = GPIO_MODE_OUTPUT_PP;
    GPIO_InitStruct.Pull  = GPIO_PULLUP;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
    HAL_GPIO_Init(W25Q64_CS_GPIOx, &GPIO_InitStruct);
    W25Q64_CS_1();
}
...
/************************************************************************
// 函 数 名:      void  Read_aAsc40_Dot(u8 chr,u8 *dot)
// 函数功能描述:  从W25Q64中读取一个ASCII字符的40行点阵                
// 参数描述:      u8 chr :  要读取的一个ASCII字符
                  u8 *dot:  读到的ASC32字符的40行点阵, 80字节
// 返 回 值:      无
// 返回值描述:    无
// 全局变量:      无
// 函数状态：     完成测试
...
// 修改日期：     201x-xx-xx
************************************************************************/ 
void  Read_aAsc40_Dot(uint32_t chr,uint8_t *dot)
{
    uint32_t  asc_dot_addr;

    asc_dot_addr = (chr * ASC40_nByte) + ASC40_START_ADRESS; // ascii点阵的开始地址  
    W25Q64_Read(dot,asc_dot_addr,ASC40_nByte);
}
