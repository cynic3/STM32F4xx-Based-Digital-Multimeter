...
/****************************************************************************
// 函 数 名:      Gui_Circle_Fill(u16 X0,u16 Y0,u16 R,u16 fc)
// 函数功能描述:  Bresenham算法 画圆--实心圆
// 参数描述:      u16 X0,u16 Y0:颜色坐标; u16 R:半径; u16 fc:颜色值
// 返 回 值:      无
// 返回值描述:    无
// 全局变量:      无
...
// 修改日期：     201x-xx-xx
****************************************************************************/
void Gui_Circle_Fill(uint16_t X0,uint16_t Y0,uint16_t R,uint16_t fc)
{
    int  x, y, d;
    ...
    while (x < y)
    {
        Gui_Draw_Line(X0+x, Y0+y, X0+x, Y0-y, fc);
        Gui_Draw_Line(X0-x, Y0+y, X0-x, Y0-y, fc);
        Gui_Draw_Line(X0-y, Y0+x, X0+y, Y0+x, fc);
        Gui_Draw_Line(X0-y, Y0-x, X0+y, Y0-x, fc);
...
//////////////////////////////////////////////////////////////////////////
//显示32*32的文字的函数
void Gui_Draw_A_Word_32x32(uint16_t x, uint16_t y, uint16_t forcolor, uint16_t bkcolor,const uint8_t *word)
{
    uint32_t  i,j,k;
    uint8_t tmp;
    
    Lcd_Set_Region(x,y,x+31,y+31);//设置显示区域为32*32
    for(i=0;i<32;i++)             //一共32行
    {
        for(j=0;j<4;j++)          // 每行写4个字节 
        {
            tmp=*word++;
            for(k=0;k<8;k++)      // 每个字节 8 个点（像素）   
            {
                if(tmp & 0x80) { *(__IO uint16_t *)(Bank1_LCD_D) = forcolor; } // 写字符的像素点
                else           { *(__IO uint16_t *)(Bank1_LCD_D) = bkcolor;  } // 写字符背景的像素点
                tmp<<=1;
            }
        }
    }
    Lcd_Set_Region(0,0,X_MAX_PIXEL-1,Y_MAX_PIXEL-1);  // 还原整屏作为显示区域
}

/****************************************************************************
* 名    称：void LCD_DrawPicture(u16 StartX,u16 StartY,u8 *pic)
* 功    能：在指定位置显示一幅图
* 入口参数： StartX     行起始座标
*           StartY     列起始座标
*           pic        图片头指针
* 出口参数：无
* 说    明：
* 调用方法：LCD_DrawPicture(0,0,(u8*)demo);
****************************************************************************/
void LCD_Draw_Picture(uint16_t xstr,uint16_t ystr,uint8_t *pic)	    //画图
{
    uint32_t  i,j;
    uint16_t  tmp16;

    Lcd_Set_Region(xstr,ystr,xstr+PICTURE_W-1,ystr+PICTURE_H-1);//设置显示区域为160*200
    ...
    {   
        ...
        { 
            ...
            tmp16  += *pic;
            pic += 2;
            *(__IO uint16_t *)(Bank1_LCD_D) = tmp16;
        }
    } 
    Lcd_Set_Region(0,0,X_MAX_PIXEL-1,Y_MAX_PIXEL-1);  // 还原整屏作为显示区域
}
...
void Gui_Draw_Dot40X40(uint16_t x, uint16_t y, uint16_t forcolor, uint16_t bkcolor, const uint8_t *word)
{
    ...
    for(i=0;i<40;i++)   // 40行
    {
        for (j=0;j<5;j++) // 1行内包含5个字节
        {
            tmp = *word;
            for(k=0;k<8;k++) // 1个字节内包含8个像素
            {
                if(tmp & 0x80) { *(__IO uint16_t *)(Bank1_LCD_D) = forcolor; }
                else           { *(__IO uint16_t *)(Bank1_LCD_D) = bkcolor;  }
                tmp <<= 1; 
